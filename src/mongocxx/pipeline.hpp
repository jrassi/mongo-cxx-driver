...
class MONGOCXX_API pipeline {
   public:
    ...
    ///
    /// @param bucket_args
    ///   The specification for the bucket operation.  The required fields @c groupBy and @c
    ...
    ///   The specification for the bucket_auto operation.  This required fields @c groupBy and @c
    ...
    ///
    /// @param geo_near_args
    ///   The specification for the geo_near operation.  The required fields @c near and @c
    ...
    ///
    /// @see https://docs.mongodb.com/master/release-notes/3.4-reference/#pipe._S_graphLookup
    ///
    /// @param graph_lookup_args
    ///   The specification for the graph_lookup operation.  The required fields @c from, @c
    ///   connectFromField, @c startWith, @c connectToField, and @as must be included.
    ///
    ...
    ///
    /// @param group_args
    ///   The specification for the group operation.  The required field @c _id must be included.
    ///
    ...
    ///
    /// @param lookup_args
    ///   The specification for the lookup operation.  The required fields @c from, @c localField,
    ///   @c foreignField, and @c as must be included.
    ///
    ...
    ///   The specification for the replace_root operation.  The required field @c newRoot must be
    ///   included.
    ///
    ...
    ///
    /// @param field_expression
    ///   The expression to group by, as a string.  To specify a field path, prefix the field name
    ///   with a dollar sign $ and enclose it in quotes. The expression can not evaluate to an
    ///   object.
    ///
